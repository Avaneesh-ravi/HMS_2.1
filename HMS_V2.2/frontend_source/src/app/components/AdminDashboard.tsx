        ...
        try {
          ...
          if (data.success && data.data) {
            ...
            const fetchedResponses: FeedbackResponse[] = data.data.map((item: any) => ({
              ...
              ratings: {}, // Can be mapped later if needed
              yesNoAnswers: {},
              appreciations: [],
              whyChooseUs: []
            }));
                  ...
                  <div className="bg-white rounded-2xl shadow-md p-6 border-l-4 border-blue-500 hover:shadow-lg transition-shadow md:col-span-2 lg:col-span-1">
                    ...
                    <div className="flex items-start gap-6">
                      <div className="flex flex-col items-center">
                        ...
                        <div className="text-xs text-gray-500 mt-1">1,247 ratings</div>
                      </div>
              ...
              <div className="pb-24">
                ...
                <div className="bg-white rounded-2xl shadow-md p-6 border-l-4 border-teal-500">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <button className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-all shadow-md">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(selectedResponse.ratings).map(([key, value]) => (
                    value > 0 && (
                      <div key={key} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <span className="text-sm text-gray-700 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                        <span className="font-semibold text-teal-600">{value}/5</span>
                      </div>
                    )
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      selectedResponse.yesNoAnswers.wouldRecommend ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                      {selectedResponse.yesNoAnswers.wouldRecommend ? 'Yes' : 'No'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-700">Cost Explained</span>
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      selectedResponse.yesNoAnswers.costExplained ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                      {selectedResponse.yesNoAnswers.costExplained ? 'Yes' : 'No'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-700">Cleanliness Issue</span>
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      selectedResponse.yesNoAnswers.cleanlinessIssue ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                    }`}>
                      {selectedResponse.yesNoAnswers.cleanlinessIssue ? 'Yes' : 'No'}
                    </span>
                  </div>
                </div>
              </div>
